...
async def test_dns_hijack_detection():
    ...
    mock_getaddrinfo = AsyncMock(return_value=[(0, 0, 0, 0, ("185.199.108.133", 0))])

    with patch(
        "asyncio.get_running_loop", return_value=AsyncMock(getaddrinfo=mock_getaddrinfo)
    ):
...
async def test_dns_dropped():
    ...
    with patch("diagnosis.dns.send_dns_query", AsyncMock(return_value=None)):
        ...
        with patch(
            "diagnosis.dns.test_doh_resolution", AsyncMock(return_value="dropped")
        ):
...
async def test_dns_ok():
    ...
    mock_getaddrinfo = AsyncMock(return_value=[(0, 0, 0, 0, ("8.8.8.8", 0))])
    ...
    with patch(
        ...
        with patch(
            "diagnosis.dns.send_dns_query", AsyncMock(return_value=mock_udp_response)
        ):
            with patch(
                "diagnosis.dns.test_doh_resolution", AsyncMock(return_value="ok")
            ):

                with patch(
                    "asyncio.get_running_loop",
                    return_value=AsyncMock(
                        getaddrinfo=AsyncMock(side_effect=socket.gaierror)
                    ),
                ):
                    result = await test_dns_layer()
...
async def test_dns_unknown_anomaly():
    ...
    with patch(
        "asyncio.get_running_loop",
        return_value=AsyncMock(
            getaddrinfo=AsyncMock(return_value=[(0, 0, 0, 0, ("8.8.8.8", 0))])
        ),
    ):
        with patch(
            "diagnosis.dns.send_dns_query", AsyncMock(return_value=mock_udp_response)
            ...
            with patch(
                "diagnosis.dns.test_doh_resolution", AsyncMock(return_value="unknown")
            ):
