...
async def check_direct_health():
    ...
    resolver = __import__("core.network", fromlist=["get_resolver"]).get_resolver(
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                async with session.get(
                    "https://www.google.com/generate_204", allow_redirects=False
                ) as resp:
            ...
            try:
                start = time.time()
                async with session.get(
                    "https://www.youtube.com/generate_204", allow_redirects=False
                ) as resp:
                    latency = round((time.time() - start) * 1000, 2)
                    if resp.status in (200, 204):
    ...
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            if country == "IR":
                ...
                log(
                    f"Geolocation: {country}. Already bypassing or outside Iran.",
                    "PASS",
                )
                return False
            else:
                log(
                    "Geolocation undetermined. Assuming inside Iran to run tests.",
                    "WARN",
                )
                return True
    except Exception as e:
...
async def scan_clean_cdn_ips(
    cdn_provider="cloudflare", worker_host=None, worker_path="/", count=3
):
    ...
    async def test_ip(ip):
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(ip, 443), timeout=2
            )
            latency = round((time.time() - start) * 1000, 2)
            writer.close()
            ...
            ssl_reader, ssl_writer = await asyncio.open_connection(
                ...
                port=443,
                ssl=ctx,
                server_hostname=worker_host or "speed.cloudflare.com",
            )
            ...
            if (
                b"HTTP/1.1 101" in resp_data
                or b"HTTP/1.1 426" in resp_data
                or b"HTTP/1.1 400" in resp_data
            ):
                return (ip, latency)
            return None
        except Exception:
