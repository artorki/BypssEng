...
async def test_dpi_layer():
    ...
    try:
                ...
                try:
                    ...
                    async with session.get(url, allow_redirects=False) as resp:
                        ...
                        if name == "youtube":
                            ...
                            elif resp.status in (301, 302) and (
                                "consent.google.com" in loc or "www.google.com" in loc
                            ):
                                results[name] = {"status": "ok", "latency": latency}
                            elif "10.10.34.34" in loc or (
                                b"filter" in body.lower() and b"blocked" in body.lower()
                            ):
                                results[name] = {
                                    "status": "blocked",
                                    "latency": latency,
        ...
        if ggl_res in ("blocked", "unknown", "timeout", "error") and yt_res in (
            "blocked",
            "unknown",
            "timeout",
            "error",
