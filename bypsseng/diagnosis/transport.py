...
async def test_vpn_ports():
    """Tests if standard VPN ports are blocked."""
...
async def test_udp_status():
    """
    Section 23: Includes UDP latency in evidence.
    Section 27: Returns udp_unknown for conflicting states.
    """
    results = []
    latencies = []
    ...
    for ip in CONFIG.targets.external_ips:
        ...
        if res is not None and res.get('ancount', 0) > 0 and res.get('txid') == res.get('expected_txid'):
            results.append('ok')
            if 'latency' in res: latencies.append(res['latency'])
        elif res is None:
    ...
    evidence = [f"udp_latency={avg_latency}ms" if avg_latency else "udp_latency=timeout"]
