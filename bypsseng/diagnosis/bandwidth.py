...
async def test_throttling():
            ...
            try:
                ...
                async with session.get(url) as response:
                    ...
                    async for chunk in response.content.iter_chunked(8192):
                        ...
                        if (
                            downloaded >= CONFIG.thresholds.speed_test_bytes
                            or (time.time() - start)
                            >= CONFIG.thresholds.speed_test_max_duration
                        ):
                            break
                return (
                    (downloaded * 8 / 1000) / (time.time() - start)
                    if downloaded > 0
                    else 0
                )
    ...
    try:
        async with aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=CONFIG.intervals.http_timeout),
            connector=connector,
        ) as session:
            nat_speeds = await asyncio.gather(
                *[
                    download_test(nat_url, session)
                    for nat_url in CONFIG.targets.national_speed_urls
                    for _ in range(CONFIG.thresholds.speed_test_samples)
                ]
            )
            int_speeds = await asyncio.gather(
                *[
                    download_test(int_url, session)
                    for int_url in CONFIG.targets.international_speed_urls
                    for _ in range(CONFIG.thresholds.speed_test_samples)
                ]
            )

            nat_speed = (
                statistics.median([s for s in nat_speeds if s > 0])
                if any(nat_speeds)
                else 0
            )
            int_speed = (
                statistics.median([s for s in int_speeds if s > 0])
                if any(int_speeds)
                else 0
            )

            evidence = [
                f"nat_speed={nat_speed:.2f}kbps",
                f"int_speed={int_speed:.2f}kbps",
            ]
            ...
            if (
                int_speed < CONFIG.thresholds.speed_kbps_severe
                and nat_speed > int_speed * 3
            ):
                return DiagnosisResult(
