    ...
    if system == "windows":
        try:
            ...
            try:
                ...
                w.open()
                log(
                    "  -> [Kernel DPI] Listening for RST packets via WinDivert...",
                    "INFO",
                )
                ...
                def listen():
                    try:
                        ...
                        if packet.tcp.rst:
                            ...
                            logger.debug(
                                f"[Kernel DPI] RST captured. TTL: {ttl}, Seq: {seq}"
                            )
                ...
                try:
                    result = await asyncio.wait_for(
                        loop.run_in_executor(None, listen), timeout=2.0
                    )
                    if result and result[0]:
                        ttl_val = result[1]
                        seq_val = result[2]
                        log(
                            "  -> [Kernel DPI] Injected RST with abnormal TTL detected! Confirmed DPI attack.",
                            "FAIL",
                        )
                        ...
                        log(
                            "  -> [Kernel DPI] No abnormal RST packets detected.",
                            "PASS",
                        )
                        return DiagnosisResult(
                ...
                log(
                    f"  -> [Kernel DPI] WinDivert driver error: {e}. Falling back.",
                    "WARN",
                )
