...
class Orchestrator:
    def __init__(
        ...
        self.local_http_port = (
            runtime_session.local_http_port if runtime_session else 10809
        )
            ...
            try:
                ...
                if current_state in [EngineState.BASELINE, EngineState.RESELECTING]:
                    ...
                    if await pm.test_current_proxy_health(self.local_http_port):
                        ...
                        await asyncio.sleep(
                            CONFIG.intervals.test_loop + random.uniform(0, 15)
                        )
                    else:
                        log(
                            "Health check failed during monitoring. Entering DEGRADED state.",
                            "WARN",
                        )
                    ...
                    log(
                        f"Unknown state {current_state}. Resetting to RESELECTING.",
                        "WARN",
                    )
                    self.sm.transition(EngineState.RESELECTING)
                ...
                log(
                    f"Orchestrator error: {e}. Forcing DEGRADED state for recovery.",
                    "ERROR",
                )
    ...
    async def diagnose(self):
        ...
        if self.progress_callback:
            await self.progress_callback({"phase": "start", "status": "running"})
        ...
        if await check_captive_portal():
            log(
                "CRITICAL: Captive Portal detected! Please authenticate in your browser.",
                "FAIL",
            )
        ...
        if is_healthy:
            log(
                "Network is healthy (Direct access succeeded). No bypass needed.",
                "PASS",
            )
            self.states = {
                "ip": {"internal": True, "external": True},
                "dns": "dns_ok",
                "dpi": "dpi_none",
                "speed": "speed_ok",
            }
            ...
            if self.report_callback:
                self.report_callback(
                    self.states, "healthy", ["direct_access_ok"], "healthy"
                )

            log(
                f"Waiting for {CONFIG.intervals.test_loop} seconds before next diagnosis cycle...",
                "INFO",
            )
            await asyncio.sleep(CONFIG.intervals.test_loop + random.uniform(0, 15))
            self.sm.transition(EngineState.RESELECTING)
        ...
        if self.progress_callback:
            await self.progress_callback(
                {"phase": 1, "name": "Network & Routing", "status": "running"}
            )
        ip_s = await test_ip_layer()
        if self.progress_callback:
            await self.progress_callback({"phase": 1, "status": "done", "data": ip_s})

        if self.progress_callback:
            await self.progress_callback(
                {"phase": 2, "name": "DNS Layer", "status": "running"}
            )
        dns_res = await test_dns_layer()
        if self.progress_callback:
            await self.progress_callback(
                {"phase": 2, "status": "done", "data": dns_res.condition}
            )

        if self.progress_callback:
            await self.progress_callback(
                {"phase": 4, "name": "DPI Inspection", "status": "running"}
            )
        try:
        ...
        if self.progress_callback:
            await self.progress_callback(
                {"phase": 4, "status": "done", "data": dpi_res.condition}
            )

        if self.progress_callback:
            await self.progress_callback(
                {"phase": 5, "name": "Bandwidth", "status": "running"}
            )
        speed_res = await test_throttling()
        if self.progress_callback:
            await self.progress_callback(
                {"phase": 5, "status": "done", "data": speed_res.condition}
            )

        if self.progress_callback:
            await self.progress_callback(
                {"phase": 3, "name": "UDP Status", "status": "running"}
            )
        udp_res = await test_udp_status()
        if self.progress_callback:
            await self.progress_callback(
                {"phase": 3, "status": "done", "data": udp_res.condition}
            )
        ...
        if self.progress_callback:
            await self.progress_callback(
                {"phase": "final", "status": "done", "states": self.states}
            )
    ...
    async def select_and_connect(self):
        ...
        log(
            f"Decision Engine Result: {primary_diagnosis.condition} (Confidence: {primary_diagnosis.confidence})",
            "SOL",
        )

        self.sm.transition(EngineState.STARTING)

        success, explanation = await self.bypass_executor(
            self.states, primary_diagnosis
        )

        if explanation:
            log(
                f"Decision Explainability: Selected {explanation.selected} over {explanation.alternatives}. Reasons: {explanation.evidence}",
                "INFO",
            )
            ...
            if self.telemetry_db:
                await self.telemetry_db.insert_decision_telemetry(
                    ...
                    selected_strategy=explanation.selected,
                    score=0.0,
                    result="success" if success else "failed",
                    explanation={
                        "alternatives": explanation.alternatives,
                        "reasons": explanation.evidence,
                    },
                )
        ...
        if success:
            ...
            if self.telemetry_db:
                await self.telemetry_db.record_strategy_outcome(
                    "all_failed", primary_diagnosis.condition, False
                )
            if self.report_callback:
                self.report_callback(
                    self.states, "failed", [primary_diagnosis.condition], "failed"
                )
            self.sm.transition(EngineState.DEGRADED)
