...
class IntervalsConfig(BaseModel):
    """Configuration for network testing and loop intervals."""
    test_loop: int = Field(default=120, description="Interval between full diagnosis cycles when network is stable.")
    blackout_loop: int = Field(default=300, description="Interval between tests when network is completely blacked out.")
    ...
    http_timeout: int = Field(default=10, description="Timeout for general HTTP requests.")
    global_test_timeout: int = Field(default=45, description="Maximum time allowed for a full diagnosis phase.")

    @field_validator('tcp_timeout', 'dns_timeout', 'http_timeout', 'global_test_timeout', 'test_loop', 'blackout_loop')
    @classmethod
    def check_positive_timeout(cls, v: int) -> int:
        """Section 57: Ensure all time intervals are strictly positive."""
        if v <= 0:
            raise ValueError("Timeout and loop intervals must be greater than 0")
        return v

class TargetsConfig(BaseModel):
    """Target endpoints and IPs for network diagnosis."""
    ...
    dns_candidates: List[str] = [
        ...
        "94.140.14.14", "94.140.15.15", "217.218.127.127", "217.218.155.155", "91.92.255.244"
    ]

class ThresholdsConfig(BaseModel):
    """Thresholds for speed and bandwidth diagnosis."""
    speed_kbps_severe: int = Field(default=20, description="Threshold for severely throttled speed.")
    speed_kbps_slow: int = Field(default=500, description="Threshold for slow speed.")
    speed_test_samples: int = Field(default=3, description="Number of samples to take for speed tests.")
    speed_test_bytes: int = Field(default=2000000, description="Amount of data to download for speed test.")
    speed_test_max_duration: int = Field(default=5, description="Maximum duration in seconds for a speed test.")
...
class CdnRangesConfig(BaseModel):
    """IP ranges for CDN providers used in Clean IP Scanner."""
...
class AppConfig(BaseModel):
    """
    Section 87: Central Configuration Model.
    Initialized with defaults to run out-of-the-box.
    """
