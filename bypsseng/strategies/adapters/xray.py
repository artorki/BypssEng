...
class XrayStrategy(Strategy):
    ...
    async def prepare(self) -> tuple:
        ...
        common_inbounds = [
            {"port": self.local_socks_port, "listen": "127.0.0.1", "protocol": "socks", "settings": {"udp": True}, "sniffing": {"enabled": True, "destOverride": ["http", "tls", "quic"]}},
        ...
        def make_shadowtls_outbound(c, tag="proxy"):
            return apply_fragmentation({"tag": tag, "protocol": "shadowtls", "settings": {"servers": [{"address": c["shadowtls_server_ip"], "port": c["shadowtls_port"], "password": c["shadowtls_password"]}]}, "streamSettings": {"network": "tcp", "security": "tls", "tlsSettings": {"serverName": c.get("shadowtls_sni", "") or get_less_popular_sni(), "fingerprint": dynamic_tls["fingerprint"], "minVersion": "1.2"}}})
        ...
        def make_vmess_outbound(c, tag="proxy"):
            ...
            if c["vmess_type"] == "ws": stream["wsSettings"] = {"path": c["vmess_path"], "headers": {"Host": c["vmess_host"] or c["vmess_sni"]}}
            elif c["vmess_type"] == "grpc": stream["grpcSettings"] = {"serviceName": c.get("vmess_service_name", "")}
            elif c["vmess_type"] == "http": stream["httpSettings"] = {"path": c["vmess_path"], "host": [c["vmess_host"]]}
            if c["vmess_tls"] == "tls":
                stream["security"] = "tls"; stream["tlsSettings"] = {"serverName": c["vmess_sni"], "fingerprint": dynamic_tls["fingerprint"], "minVersion": "1.2"}
                ob["streamSettings"] = stream; return apply_fragmentation(ob)
            else:
                stream["security"] = "none"; ob["streamSettings"] = stream; return apply_tcp_nodelay(ob)

        def make_ss_outbound(c, tag="proxy"):
            return apply_tcp_nodelay({"tag": tag, "protocol": "shadowsocks", "settings": {"servers": [{"address": c["ss_server_ip"], "port": c["ss_port"], "method": c["ss_method"], "password": c["ss_password"]}]}})
            ...
            if c["protocol"] == "vless":
                if c.get("vless_security") == "reality": return make_vless_reality_outbound(c, tag)
                    ...
                    if c.get("vless_security") == "tls":
                        ob["streamSettings"]["security"] = "tls"; ob["streamSettings"]["tlsSettings"] = {"serverName": c.get("vless_sni", ""), "fingerprint": dynamic_tls["fingerprint"], "minVersion": "1.2"}; return apply_fragmentation(ob)
                    else: ob["streamSettings"]["security"] = "none"; return apply_tcp_nodelay(ob)
        ...
        if creds["protocol"] == "warp_over_reality":
            ...
            warp_outbound = {
                ...
                "settings": {
                    ...
                    "reserved": warp_data.get("reserved", [0, 0, 0]), "kernelMode": False
                }
            }
        ...
        if creds["protocol"] == "cloudflare_worker":
            ...
            if not worker_data or not worker_data.get("id") or not worker_data.get("host"): return None, None
            worker_host = worker_data["host"]; worker_id = worker_data["id"]; worker_path = worker_data.get("path", "/ws")
            cf_ip = None
            ...
            if clean_ips: cf_ip = clean_ips[0]
            if not cf_ip: return None, None
            config = {
                "log": {"loglevel": "warning"}, "dns": common_dns, "inbounds": common_inbounds,
                "outbounds": [{"tag": "proxy", "protocol": "vless", "mux": mux_settings, "settings": {"vnext": [{"address": cf_ip, "port": 443, "users": [{"id": worker_id, "encryption": "none"}]}]}, "streamSettings": {"network": "ws", "security": "tls", "tlsSettings": {"serverName": worker_host, "fingerprint": dynamic_tls["fingerprint"], "minVersion": "1.2"}, "wsSettings": {"path": worker_path, "headers": {"Host": worker_host}}}}],
                "routing": routing_rules
            }
        ...
        if use_balancer:
            ...
            for idx, c in enumerate(self.all_configs):
                ...
                if ob: ob = apply_fragmentation(ob); config["outbounds"].append(ob); balancer_tags.append(tag)
            if balancer_tags:
                ...
                config["observatory"] = {"subjectSelect": balancer_tags, "probeURL": "https://www.gstatic.com/generate_204", "probeInterval": "30s", "enableConcurrency": True}
            else:
