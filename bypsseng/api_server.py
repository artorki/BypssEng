...
async def manual_connect_api(request):
    ...
    success = await BypssEng.manual_connect(link)
    if success:
        return web.json_response({"status": "connecting"})
    else:
        return web.json_response({"error": "Invalid config or engine not ready"}, status=400)
...
async def export_diagnostics(request):
    diag_data = {
        "system_info": {"os": os.name, "python_version": sys.version},
        "network_report": {},
        "recent_logs": [],
        "decision_history": [],
    }
...
async def create_app():
    ...
    app.add_routes([
        ...
        web.post('/api/configs/connect', manual_connect_api),
